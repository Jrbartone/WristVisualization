...
function tubeControl(e) {
	switch(e.key){
		case "w":
			outerMotion.advancement = outerMotion.advancement + .1;
			break;
		case "s":
			outerMotion.advancement = outerMotion.advancement - .1;
			break;
		...
		case "q":
			if(outerMotion.displacement > 0) {
				outerMotion.displacement = outerMotion.displacement - .01;
			}
			break;
		case "e":
			outerMotion.displacement = outerMotion.displacement + .01;
			break;
		case "i":
			innerMotion.advancement = innerMotion.advancement + .1;
			break;
		case "k":
			innerMotion.advancement = innerMotion.advancement - .1;
			break;
		...
		case "u":
			if(innerMotion.displacement > 0) {
				innerMotion.displacement = innerMotion.displacement - .01;
			}
			break;
		case "o":
			innerMotion.displacement = innerMotion.displacement + .01;
...
function draw() {
	...
	let points = kinematicsPoints(outerMotion.displacement, radians, outerMotion.advancement);
	let rots = rotations(outerMotion.displacement, radians, outerMotion.advancement);
	let innerRadians = innerMotion.rotation * PI / 180.0;
	let innerPoints = innerKinematicsPoints(innerMotion.displacement, innerRadians, innerMotion.advancement);
	let innerRots = innerRotations(innerMotion.displacement, innerRadians, innerMotion.advancement);
